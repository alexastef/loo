...
module.exports = function (app) {
    ...
    if (source === "home") {
      ...
      db.Bathroom.findAll({
        ...
        bathroomsDataValues.forEach((dbBathroom) => {
          const matchingGooglePlace = places.find(
            (detailedPlace) => detailedPlace.place_id === dbBathroom.place_id
          );
  ...
  app.get("/api/oneplace/:place_id", async function (req, res) {
    ...
    const fields =
      "name,formatted_phone_number,formatted_address,geometry,photos,place_id";
  ...
  app.post("/api/bathroom", function (req, res) {
    ...
    db.Bathroom.create(req.body)
      .then(function (newLoo) {
        res.json(newLoo);
      })
